...
def list_user_tweets(id: int, db: Session):
...
def update_user_tweet(id: int, tweet: TweetCreate, db: Session, owner_id: int):
...
def delete_user_tweet(id: int, db: Session, owner_id: int):
    existing_tweet = db.query(Tweet).filter(Tweet.id == id)
